...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A basalt sill intruded concordantly between sandstone layers.');
    ...
    if (model.intrusions[0].subtype !== 'sill') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype sill, got ${model.intrusions[0].subtype}`);
    const lithI02 = model.intrusions[0].lithology || model.intrusions[0].rock_type || '';
    if (lithI02 !== 'basalt') throw new Error(`FAIL ${SCENARIO_ID}: expected basalt lithology, got ${lithI02}`);
