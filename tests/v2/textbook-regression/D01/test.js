...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A plunging anticline with the oldest rocks in the core, plunging 10 degrees north, interlimb angle 120 degrees.');
    ...
    if (model.events[0].type !== 'fold') throw new Error(`FAIL ${SCENARIO_ID}: expected event type fold, got ${model.events[0].type}`);
    if (model.events[0].subtype !== 'anticline') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype anticline, got ${model.events[0].subtype}`);
    if (Math.abs(model.events[0].plunge - 10) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected plunge 10, got ${model.events[0].plunge}`);
    if (Math.abs(model.events[0].interlimb_angle - 120) > 5) throw new Error(`FAIL ${SCENARIO_ID}: expected interlimb_angle 120, got ${model.events[0].interlimb_angle}`);
