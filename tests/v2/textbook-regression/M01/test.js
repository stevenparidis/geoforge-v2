...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A porphyry copper-gold deposit centred on a causative stock.');
    ...
    if (model.mineralisation[0].subtype !== 'porphyry') throw new Error(`FAIL ${SCENARIO_ID}: expected porphyry, got ${model.mineralisation[0].subtype}`);
