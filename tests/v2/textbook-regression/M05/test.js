...
async function run() {
  ...
  try {
    ...
    await textarea.fill('An epithermal gold-silver deposit with low-sulphidation quartz veins.');
    ...
    if (model.mineralisation[0].subtype !== 'epithermal') throw new Error(`FAIL ${SCENARIO_ID}: expected epithermal, got ${model.mineralisation[0].subtype}`);
    ...
    if (!metalsM.some(m => String(m).includes('Ag'))) throw new Error(`FAIL ${SCENARIO_ID}: expected Ag`);
