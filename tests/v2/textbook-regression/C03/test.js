...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A limestone-sandstone stack intruded by a granite laccolith.');
    ...
    if (!model.layers || model.layers.length < 2) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 2 layers, got ${model.layers ? model.layers.length : 0}`);
    if (!model.intrusions || model.intrusions.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 intrusion`);
    if (model.intrusions[0].subtype !== 'laccolith') throw new Error(`FAIL ${SCENARIO_ID}: expected laccolith, got ${model.intrusions[0].subtype}`);
    const lithC03 = model.intrusions[0].lithology || model.intrusions[0].rock_type || '';
    if (lithC03 !== 'granite') throw new Error(`FAIL ${SCENARIO_ID}: expected granite, got ${lithC03}`);
