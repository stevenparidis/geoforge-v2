...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A reverse fault dipping 30 degrees west with 0.3m heave.');
    ...
    if (model.events[0].subtype !== 'reverse') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype reverse, got ${model.events[0].subtype}`);
    if (Math.abs(model.events[0].dip - 30) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected dip 30, got ${model.events[0].dip}`);
