...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A basalt dyke cutting vertically through sandstone and limestone layers.');
    ...
    if (model.intrusions[0].subtype !== 'dyke') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype dyke, got ${model.intrusions[0].subtype}`);
    const lithI01 = model.intrusions[0].lithology || model.intrusions[0].rock_type || '';
    if (lithI01 !== 'basalt') throw new Error(`FAIL ${SCENARIO_ID}: expected basalt lithology, got ${lithI01}`);
