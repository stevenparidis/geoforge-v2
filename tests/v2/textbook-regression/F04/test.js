...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A right-lateral (dextral) strike-slip fault.');
    ...
    const subtypeF04 = model.events[0].subtype || '';
    if (!subtypeF04.includes('dextral') && !subtypeF04.includes('strike_slip_dextral') && !subtypeF04.includes('strike-slip')) throw new Error(`FAIL ${SCENARIO_ID}: expected dextral strike-slip subtype, got ${subtypeF04}`);
    if (model.events[0].dip !== undefined && Math.abs(model.events[0].dip - 90) > 5) throw new Error(`FAIL ${SCENARIO_ID}: expected dip ~90 for strike-slip, got ${model.events[0].dip}`);
