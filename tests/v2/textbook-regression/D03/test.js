...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A monocline - a one-limbed fold where strata step down to one side.');
    ...
    if (model.events[0].subtype !== 'monocline') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype monocline, got ${model.events[0].subtype}`);
