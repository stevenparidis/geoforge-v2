...
async function run() {
  ...
  try {
    ...
    await textarea.fill('An angular unconformity cut by a subsequent normal fault.');
    ...
    if (!model.unconformities || model.unconformities.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 unconformity`);
    if (model.unconformities[0].subtype !== 'angular') throw new Error(`FAIL ${SCENARIO_ID}: expected angular, got ${model.unconformities[0].subtype}`);
