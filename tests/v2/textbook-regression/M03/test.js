...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A volcanogenic massive sulphide (VMS) deposit at the paleo-seafloor.');
    ...
    if (model.mineralisation[0].subtype !== 'vms') throw new Error(`FAIL ${SCENARIO_ID}: expected vms, got ${model.mineralisation[0].subtype}`);
    const metalsM = Array.isArray(model.mineralisation[0].metals) ? model.mineralisation[0].metals : String(model.mineralisation[0].metals || '').split(/[-,]/);
    if (!metalsM.some(m => String(m).includes('Cu'))) throw new Error(`FAIL ${SCENARIO_ID}: expected Cu`);
    if (!metalsM.some(m => String(m).includes('Zn'))) throw new Error(`FAIL ${SCENARIO_ID}: expected Zn`);
