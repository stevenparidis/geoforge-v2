...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A skarn deposit at the contact between granite and limestone.');
    ...
    if (model.mineralisation[0].subtype !== 'skarn') throw new Error(`FAIL ${SCENARIO_ID}: expected skarn, got ${model.mineralisation[0].subtype}`);
    ...
    if (!metalsM.some(m => String(m).includes('Fe'))) throw new Error(`FAIL ${SCENARIO_ID}: expected Fe`);
