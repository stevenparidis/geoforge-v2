...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A nonconformity where sedimentary rocks overlie a granite basement.');
    ...
    if (model.unconformities[0].subtype !== 'nonconformity') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype nonconformity, got ${model.unconformities[0].subtype}`);
    const basementLayer = model.layers && model.layers.find(l => l.lithology === 'granite');
    if (!basementLayer) throw new Error(`FAIL ${SCENARIO_ID}: expected a granite basement layer`);
