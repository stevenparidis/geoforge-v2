...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A laccolith of granite doming the overlying sedimentary layers.');
    ...
    if (model.intrusions[0].subtype !== 'laccolith') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype laccolith, got ${model.intrusions[0].subtype}`);
    const lithI04 = model.intrusions[0].lithology || model.intrusions[0].rock_type || '';
    if (lithI04 !== 'granite') throw new Error(`FAIL ${SCENARIO_ID}: expected granite lithology, got ${lithI04}`);
    if (Math.abs((model.intrusions[0].depth || 0) - 1.5) > 0.1) throw new Error(`FAIL ${SCENARIO_ID}: expected depth 1.5, got ${model.intrusions[0].depth}`);
