...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A left-lateral (sinistral) strike-slip fault, striking north-south.');
    ...
    if (!model.events || model.events.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 event, got ${model.events ? model.events.length : 0}`);
    if (model.events[0].type !== 'fault') throw new Error(`FAIL ${SCENARIO_ID}: expected event type fault, got ${model.events[0].type}`);
    const subtypeF05 = model.events[0].subtype || '';
    if (!subtypeF05.includes('sinistral') && !subtypeF05.includes('strike_slip_sinistral') && !subtypeF05.includes('strike-slip')) throw new Error(`FAIL ${SCENARIO_ID}: expected sinistral subtype, got ${subtypeF05}`);
    if (model.events[0].strike !== undefined && model.events[0].strike !== 0 && model.events[0].strike !== 360) throw new Error(`FAIL ${SCENARIO_ID}: expected N-S strike (0/360), got ${model.events[0].strike}`);

    // DOM assertions
    await page.waitForSelector('.explanation-strip', { timeout: 10000 });
    await page.waitForSelector('.hw-label', { timeout: 10000 });
