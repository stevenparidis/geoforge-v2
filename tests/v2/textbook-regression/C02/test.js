...
async function run() {
  ...
  try {
    ...
    await textarea.fill('Three layers folded into an anticline.');
    ...
    if (!model.layers || model.layers.length !== 3) throw new Error(`FAIL ${SCENARIO_ID}: expected 3 layers, got ${model.layers ? model.layers.length : 0}`);
    if (!model.events || model.events.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 event`);
    if (model.events[0].type !== 'fold') throw new Error(`FAIL ${SCENARIO_ID}: expected fold event, got ${model.events[0].type}`);
    if (model.events[0].subtype !== 'anticline') throw new Error(`FAIL ${SCENARIO_ID}: expected anticline, got ${model.events[0].subtype}`);

    // DOM assertions
    await page.waitForSelector('.explanation-strip', { timeout: 10000 });
    await page.waitForSelector('.age-badge', { timeout: 10000 });
