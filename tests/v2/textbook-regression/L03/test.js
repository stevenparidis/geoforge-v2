...
async function run() {
  ...
  try {
    ...
    await textarea.fill('Three sedimentary layers from bottom to top: limestone (oldest), sandstone, shale (youngest), each 1m thick.');
    ...
    const orders = model.layers.map(l => l.order).sort((a,b)=>a-b);
    if (orders[0] !== 0 || orders[1] !== 1 || orders[2] !== 2) throw new Error(`FAIL ${SCENARIO_ID}: expected orders 0/1/2, got ${orders}`);
