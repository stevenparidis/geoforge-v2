...
async function run() {
  ...
  try {
    ...
    await textarea.fill('An oblique-slip fault with both dip-slip and strike-slip components, dipping 45 degrees east.');
    ...
    const subtypeF06 = model.events[0].subtype || '';
    if (!subtypeF06.includes('oblique')) throw new Error(`FAIL ${SCENARIO_ID}: expected oblique subtype, got ${subtypeF06}`);
    if (Math.abs(model.events[0].dip - 45) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected dip 45, got ${model.events[0].dip}`);
