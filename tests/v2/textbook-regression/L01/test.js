...
async function run() {
  ...
  try {
    ...
    await textarea.fill('Two horizontal sandstone layers, each 1m thick.');
    ...
    if (model.events && model.events.length !== 0) throw new Error(`FAIL ${SCENARIO_ID}: expected 0 events, got ${model.events.length}`);

    // DOM: explanation strip present after interpret
    await page.waitForSelector('.explanation-strip', { timeout: 10000 });
    // DOM: age badges present (multi-layer scenario)
