...
async function run() {
  ...
  try {
    ...
    await textarea.fill('An angular unconformity where upper beds are horizontal but lower beds dip 25 degrees east.');
    ...
    if (!model.unconformities || model.unconformities.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 unconformity, got ${model.unconformities ? model.unconformities.length : 0}`);
    if (model.unconformities[0].subtype !== 'angular') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype angular, got ${model.unconformities[0].subtype}`);
    if (Math.abs((model.unconformities[0].angular_discordance || 0) - 25) > 2) throw new Error(`FAIL ${SCENARIO_ID}: expected angular_discordance 25, got ${model.unconformities[0].angular_discordance}`);

    // DOM assertions
    await page.waitForSelector('.explanation-strip', { timeout: 10000 });
    await page.waitForSelector('.time-strip-container', { timeout: 10000 });
