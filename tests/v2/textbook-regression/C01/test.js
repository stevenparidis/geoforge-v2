...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A sandstone layer cut by a normal fault with 0.5m throw.');
    ...
    if (!model.events || model.events.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 event`);
    if (model.events[0].type !== 'fault') throw new Error(`FAIL ${SCENARIO_ID}: expected fault event, got ${model.events[0].type}`);
    if (model.events[0].subtype !== 'normal') throw new Error(`FAIL ${SCENARIO_ID}: expected normal, got ${model.events[0].subtype}`);
