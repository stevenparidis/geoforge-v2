...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A syncline with the youngest rocks in the core, non-plunging.');
    ...
    if (model.events[0].subtype !== 'syncline') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype syncline, got ${model.events[0].subtype}`);
    if (model.events[0].plunge !== 0) throw new Error(`FAIL ${SCENARIO_ID}: expected plunge 0, got ${model.events[0].plunge}`);
