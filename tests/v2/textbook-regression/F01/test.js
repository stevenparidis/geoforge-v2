...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A normal fault dipping 60 degrees east with 0.5m throw.');
    ...
    if (model.events[0].subtype !== 'normal') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype normal, got ${model.events[0].subtype}`);
    if (Math.abs(model.events[0].dip - 60) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected dip 60, got ${model.events[0].dip}`);
    if (Math.abs((model.events[0].throw || 0) - 0.5) > 0.05) throw new Error(`FAIL ${SCENARIO_ID}: expected throw 0.5, got ${model.events[0].throw}`);
