...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A granite batholith emplaced at depth below the sedimentary sequence.');
    ...
    if (!model.intrusions || model.intrusions.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 intrusion, got ${model.intrusions ? model.intrusions.length : 0}`);
    if (model.intrusions[0].subtype !== 'batholith') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype batholith, got ${model.intrusions[0].subtype}`);
    const lithI03 = model.intrusions[0].lithology || model.intrusions[0].rock_type || '';
    if (lithI03 !== 'granite') throw new Error(`FAIL ${SCENARIO_ID}: expected granite lithology, got ${lithI03}`);
    if (Math.abs((model.intrusions[0].depth || 0) - 2.0) > 0.1) throw new Error(`FAIL ${SCENARIO_ID}: expected depth 2.0, got ${model.intrusions[0].depth}`);

    // DOM assertions
    await page.waitForSelector('.explanation-strip', { timeout: 10000 });
    await page.waitForSelector('.intrusion-age-tag', { timeout: 10000 });
