...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A listric fault that flattens at depth.');
    ...
    if (model.events[0].subtype !== 'listric') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype listric, got ${model.events[0].subtype}`);
