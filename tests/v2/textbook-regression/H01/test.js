...
async function run() {
  ...
  try {
    ...
    await textarea.fill('First, mudstone was deposited. Then it was folded into a syncline. A granite dyke then intruded. Finally a normal fault cut through everything.');
    ...
    if (!model.layers || model.layers.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 layer`);
    if (model.layers[0].lithology !== 'mudstone') throw new Error(`FAIL ${SCENARIO_ID}: expected mudstone, got ${model.layers[0].lithology}`);
    if (!model.events || model.events.length < 2) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 2 events, got ${model.events ? model.events.length : 0}`);
    if (!model.intrusions || model.intrusions.length < 1) throw new Error(`FAIL ${SCENARIO_ID}: expected >= 1 intrusion`);
    const syncH = model.events.find(e => e.subtype === 'syncline');
    if (!syncH) throw new Error(`FAIL ${SCENARIO_ID}: expected syncline event`);
    const faultH = model.events.find(e => e.type === 'fault' && e.subtype === 'normal');
    if (!faultH) throw new Error(`FAIL ${SCENARIO_ID}: expected normal fault event`);
    const dykeH = model.intrusions.find(i => i.subtype === 'dyke');
    if (!dykeH) throw new Error(`FAIL ${SCENARIO_ID}: expected dyke intrusion`);
    if (syncH.order >= dykeH.order) throw new Error(`FAIL ${SCENARIO_ID}: syncline order should precede dyke order`);
    if (dykeH.order >= faultH.order) throw new Error(`FAIL ${SCENARIO_ID}: dyke order should precede fault order`);
