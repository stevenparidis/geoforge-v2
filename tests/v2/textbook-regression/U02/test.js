...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A disconformity representing a 15-million-year gap in the rock record.');
    ...
    if (model.unconformities[0].subtype !== 'disconformity') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype disconformity, got ${model.unconformities[0].subtype}`);
    const gapU02 = model.unconformities[0].gap_ma || model.unconformities[0].time_gap_ma;
    if (Math.abs((gapU02 || 0) - 15) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected gap_ma 15, got ${gapU02}`);
