...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A low-angle thrust fault dipping 15 degrees north.');
    ...
    if (model.events[0].subtype !== 'thrust') throw new Error(`FAIL ${SCENARIO_ID}: expected subtype thrust, got ${model.events[0].subtype}`);
    if (Math.abs(model.events[0].dip - 15) > 1) throw new Error(`FAIL ${SCENARIO_ID}: expected dip 15, got ${model.events[0].dip}`);
