...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A sandstone layer dipping 35 degrees east, 2m thick.');
    ...
    if (!model.layers || model.layers.length !== 1) throw new Error(`FAIL ${SCENARIO_ID}: expected 1 layer, got ${model.layers ? model.layers.length : 0}`);
    if (model.layers[0].lithology !== 'sandstone') throw new Error(`FAIL ${SCENARIO_ID}: expected sandstone lithology`);
    if (Math.abs(model.layers[0].thickness - 2.0) > 0.01) throw new Error(`FAIL ${SCENARIO_ID}: expected thickness 2.0, got ${model.layers[0].thickness}`);
