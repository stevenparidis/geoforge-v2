// M07: Throw/displacement conflation (positive)
// Stub returns a normal fault with dip 45, throw 5, and validation_note set.
...
const FIXTURE = {
  meta: { name: 'Throw Displacement Conflation', description: 'A fault with throw 5m and displacement 5m, dipping at 45°.' },
  layers: [
    {
      id: 'L1', name: 'Sandstone', lithology: 'sandstone', thickness: 1.0, order: 0,
      description_source: 'A fault with throw 5m and displacement 5m, dipping at 45°.',
      field_origin: { thickness: 'inferred', lithology: 'inferred' },
    },
  ],
  events: [
    {
      id: 'E1', type: 'fault', subtype: 'normal', strike: 0, dip: 45, dip_direction: 90, throw: 5, order: 0,
      description_source: 'A fault with throw 5m and displacement 5m, dipping at 45°.',
      field_origin: { strike: 'inferred', dip: 'stated', dip_direction: 'inferred', throw: 'stated' },
      validation_note: 'Throw equals net displacement only for vertical faults (dip = 90°). At 45° dip, net displacement ≈ throw / sin(dip) ≈ 7.1 m.',
    },
  ],
};
...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A fault with throw 5m and displacement 5m, dipping at 45°.');
    ...
    console.log(`PASS ${TEST_ID}: .validation-note-pill visible for throw/displacement conflation misconception`);
