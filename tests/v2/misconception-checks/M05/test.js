// M05: Normal fault HW up (positive)
// Stub returns a normal fault event with validation_note for HW moving up.
...
const FIXTURE = {
  meta: { name: 'Normal Fault HW Up', description: 'A normal fault where the hanging wall moves up.' },
  layers: [
    {
      id: 'L1', name: 'Sandstone', lithology: 'sandstone', thickness: 1.0, order: 0,
      description_source: 'A normal fault where the hanging wall moves up.',
  ...
  events: [
    {
      ...
      description_source: 'A normal fault where the hanging wall moves up.',
      field_origin: { strike: 'inferred', dip: 'inferred', dip_direction: 'inferred', throw: 'inferred' },
      validation_note: 'In a normal fault, the hanging wall moves DOWN relative to the footwall. Upward HW movement describes a reverse or thrust fault.',
    },
  ],
};
...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A normal fault where the hanging wall moves up.');
    ...
    console.log(`PASS ${TEST_ID}: .validation-note-pill visible for normal fault HW up misconception`);
