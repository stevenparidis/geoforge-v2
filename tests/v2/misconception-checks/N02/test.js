// N02: Valid normal fault correct HW (negative)
// Stub returns a valid normal fault with no validation_note.
...
const FIXTURE = {
  meta: { name: 'Normal Fault Valid HW', description: 'A normal fault dipping 60° east with the hanging wall on the west side (down-thrown).' },
  layers: [
    {
      id: 'L1', name: 'Sandstone', lithology: 'sandstone', thickness: 1.0, order: 0,
      description_source: 'A normal fault dipping 60° east with the hanging wall on the west side (down-thrown).',
...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A normal fault dipping 60° east with the hanging wall on the west side (down-thrown).');
    ...
    console.log(`PASS ${TEST_ID}: no .validation-note-pill for valid normal fault`);
