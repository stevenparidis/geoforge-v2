// N04: Valid thrust normal dip (negative)
// Stub returns a thrust fault with dip 20 and no validation_note.
...
const FIXTURE = {
  meta: { name: 'Thrust Valid Dip', description: 'A thrust fault dipping 20° east.' },
  layers: [
    {
      id: 'L1', name: 'Limestone', lithology: 'limestone', thickness: 1.0, order: 0,
      description_source: 'A thrust fault dipping 20° east.',
      field_origin: { thickness: 'inferred', lithology: 'inferred' },
    },
  ],
  events: [
    {
      id: 'E1', type: 'fault', subtype: 'thrust', strike: 0, dip: 20, dip_direction: 90, throw: 0.5, order: 0,
      description_source: 'A thrust fault dipping 20° east.',
...
async function run() {
  ...
  try {
    ...
    await textarea.fill('A thrust fault dipping 20° east.');
    ...
    console.log(`PASS ${TEST_ID}: no .validation-note-pill for valid thrust dip`);
